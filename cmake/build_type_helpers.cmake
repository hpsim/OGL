...
# Note: Lectem's original file has gone through several changes to adapt to our
# needs.
...
set(${PROJECT_NAME}_COVERAGE_COMPILER_FLAGS
    "-g -O0 --coverage"
    CACHE INTERNAL "")
set(${PROJECT_NAME}_COVERAGE_LINKER_FLAGS
    "--coverage"
    CACHE INTERNAL "")
...
set(${PROJECT_NAME}_LSAN_COMPILER_FLAGS
    "-g -O1 -fsanitize=leak"
    CACHE INTERNAL "")
set(${PROJECT_NAME}_LSAN_LINKER_FLAGS
    "-fsanitize=leak"
    CACHE INTERNAL "")
...
# We need to wrap all flags with `-Xcomplier` for HIP when using the NVCC
# backend
...
gko_xcompiler(${PROJECT_NAME}_NVCC_TSAN_COMPILER_FLAGS
              "-g;-O1;-fsanitize=thread;-fno-omit-frame-pointer;-fPIC")
gko_xcompiler(${PROJECT_NAME}_NVCC_TSAN_LINKER_FLAGS
              "-fsanitize=thread;-static-libtsan;-fno-omit-frame-pointer;-fPIC")
gko_xcompiler(${PROJECT_NAME}_NVCC_ASAN_COMPILER_FLAGS
              "-g;-O1;-fsanitize=address;-fno-omit-frame-pointer")
gko_xcompiler(${PROJECT_NAME}_NVCC_ASAN_LINKER_FLAGS
              "-fsanitize=address;-fno-omit-frame-pointer")
...
gko_xcompiler(${PROJECT_NAME}_NVCC_UBSAN_COMPILER_FLAGS
              "-g;-O1;-fsanitize=undefined;-static-libubsan")
gko_xcompiler(${PROJECT_NAME}_NVCC_UBSAN_LINKER_FLAGS
...
foreach(
  ...
  foreach(_TYPE IN LISTS ${PROJECT_NAME}_CUSTOM_BUILD_TYPES)
    ...
    # CMake macros, therefore it is __important__ to reset this once we are
    # done.
    ...
    elseif(_LANG STREQUAL "CXX" OR _LANG STREQUAL "HIP")
      ...
      if(DEFINED ${PROJECT_NAME}_${_LANG}_${_TYPE}_SUPPORTED)
        message(
          STATUS "Skipping ${_LANG}, not supported by build_type.cmake script")
      endif()
...
foreach(_TYPE IN LISTS ${PROJECT_NAME}_CUSTOM_BUILD_TYPES)
  ...
  if(${PROJECT_NAME}_${_TYPE}_IN_CONFIGURATION_TYPES)
    # Modify this only if using a multi-config generator some modules rely on
    # this variable to detect those generators.
