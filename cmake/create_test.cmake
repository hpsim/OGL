...
function(ginkgo_create_test test_name)
  file(RELATIVE_PATH REL_BINARY_DIR ${PROJECT_BINARY_DIR}
       ${CMAKE_CURRENT_BINARY_DIR})
  string(REPLACE "/" "_" TEST_TARGET_NAME "${REL_BINARY_DIR}/${test_name}")
  add_executable(${TEST_TARGET_NAME} ${test_name}.cpp)
  target_compile_features("${TEST_TARGET_NAME}" PUBLIC cxx_std_14)
  target_include_directories(
    "${TEST_TARGET_NAME}" PRIVATE "$<BUILD_INTERFACE:${PROJECT_BINARY_DIR}>"
  ...
  target_link_libraries(${TEST_TARGET_NAME} PUBLIC Ginkgo::ginkgo GTest::Main
                                                   GTest::GTest ${ARGN})
...
function(ginkgo_create_gbench bench_name)
  ...
  target_include_directories(
    "${TEST_TARGET_NAME}" PRIVATE "$<BUILD_INTERFACE:${PROJECT_BINARY_DIR}>"
                                  "$<BUILD_INTERFACE:${PROJECT_SOURCE_DIR}>")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME
                                                       ${bench_name})
  if(GINKGO_CHECK_CIRCULAR_DEPS)
    target_link_libraries(${TEST_TARGET_NAME}
                          PRIVATE "${GINKGO_CIRCULAR_DEPS_FLAGS}")
...
function(ginkgo_create_thread_test test_name)
  ...
  target_include_directories("${TEST_TARGET_NAME}"
                             PRIVATE "$<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}>")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME ${test_name})
  if(GINKGO_CHECK_CIRCULAR_DEPS)
    target_link_libraries(${TEST_TARGET_NAME}
                          PRIVATE "${GINKGO_CIRCULAR_DEPS_FLAGS}")
  ...
  target_link_libraries(
    ${TEST_TARGET_NAME} PRIVATE Ginkgo::ginkgo GTest::Main GTest::GTest
...
function(ginkgo_create_test_cpp_cuda_header test_name)
  ...
  target_compile_features("${TEST_TARGET_NAME}" PUBLIC cxx_std_14)
  target_include_directories(
    "${TEST_TARGET_NAME}" PRIVATE "$<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}>"
                                  "${CUDA_INCLUDE_DIRS}")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME ${test_name})
  if(GINKGO_CHECK_CIRCULAR_DEPS)
    target_link_libraries(${TEST_TARGET_NAME}
  ...
  target_link_libraries(${TEST_TARGET_NAME} PRIVATE Ginkgo::ginkgo GTest::Main
                                                    GTest::GTest ${ARGN})
  add_test(NAME ${REL_BINARY_DIR}/${test_name} COMMAND ${TEST_TARGET_NAME})
...
function(ginkgo_create_cuda_test test_name)
  ...
  cas_target_cuda_architectures(
    ${TEST_TARGET_NAME} ARCHITECTURES ${GINKGO_CUDA_ARCHITECTURES} UNSUPPORTED
    "20" "21")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME ${test_name})
...
function(ginkgo_create_mpi_test test_name num_mpi_procs)
  ...
  target_link_libraries(${TEST_TARGET_NAME} PRIVATE Ginkgo::ginkgo GTest::Main
                                                    GTest::GTest ${ARGN})
  target_link_libraries(${TEST_TARGET_NAME} PRIVATE ${MPI_C_LIBRARIES}
                                                    ${MPI_CXX_LIBRARIES})
  set(test_param
      ${MPIEXEC_NUMPROC_FLAG} ${num_mpi_procs} ${OPENMPI_RUN_AS_ROOT_FLAG}
      ${CMAKE_BINARY_DIR}/${REL_BINARY_DIR}/${test_name})
  add_test(NAME ${REL_BINARY_DIR}/${test_name} COMMAND ${MPIEXEC_EXECUTABLE}
                                                       ${test_param})
...
function(ginkgo_create_hip_test test_name)
  ...
  set_source_files_properties(${test_name}.hip.cpp
  ...
  if(HIP_VERSION GREATER_EQUAL "3.5")
    hip_add_executable(
      ...
      ${test_name}.hip.cpp
      HIPCC_OPTIONS
      ${GINKGO_HIPCC_OPTIONS}
      NVCC_OPTIONS
      ${GINKGO_HIP_NVCC_OPTIONS}
      HCC_OPTIONS
      ${GINKGO_HIP_HCC_OPTIONS}
      CLANG_OPTIONS
      ${GINKGO_HIP_CLANG_OPTIONS})
  else()
    hip_add_executable(
      ${TEST_TARGET_NAME}
      ${test_name}.hip.cpp
  ...
  target_include_directories(
    "${TEST_TARGET_NAME}"
    PRIVATE "$<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}>"
            # Only `math` requires it so far, but it's much easier to put these
            # this way.
            ${GINKGO_HIP_THRUST_PATH}
            # Only `exception_helpers` requires this so far, but it's much
            # easier to put these this way.
  ...
  if(GINKGO_HIP_AMDGPU AND GINKGO_HIP_PLATFORM MATCHES "hcc")
    foreach(target ${GINKGO_HIP_AMDGPU})
      target_link_libraries(${TEST_TARGET_NAME}
                            PRIVATE --amdgpu-target=${target})
    endforeach()
  endif()

  # GINKGO_RPATH_FOR_HIP needs to be populated before calling this for the
  # linker to include our libraries path into the executable's runpath.
