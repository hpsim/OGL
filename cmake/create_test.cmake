...
function(ginkgo_create_gbench bench_name)
  ...
  target_include_directories(
    "${TEST_TARGET_NAME}" PRIVATE "$<BUILD_INTERFACE:${PROJECT_BINARY_DIR}>"
                                  "$<BUILD_INTERFACE:${PROJECT_SOURCE_DIR}>")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME ${bench_name})
  if(GINKGO_CHECK_CIRCULAR_DEPS)
    target_link_libraries(${TEST_TARGET_NAME} PRIVATE "${GINKGO_CIRCULAR_DEPS_FLAGS}")
  endif()
...
function(ginkgo_create_thread_test test_name)
  ...
  target_link_libraries(${TEST_TARGET_NAME} PRIVATE Ginkgo::ginkgo GTest::Main GTest::GTest
...
function(ginkgo_create_test_cpp_cuda_header test_name)
  ...
  add_executable(${TEST_TARGET_NAME} ${test_name}.cpp)
  target_compile_features("${TEST_TARGET_NAME}" PUBLIC cxx_std_14)
  target_include_directories("${TEST_TARGET_NAME}" PRIVATE "$<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}>"
                                                           "${CUDA_INCLUDE_DIRS}")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME ${test_name})
  if(GINKGO_CHECK_CIRCULAR_DEPS)
    target_link_libraries(${TEST_TARGET_NAME} PRIVATE "${GINKGO_CIRCULAR_DEPS_FLAGS}")
...
function(ginkgo_create_cuda_test test_name)
  ...
  target_include_directories("${TEST_TARGET_NAME}"
                             PRIVATE "$<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}>")
  cas_target_cuda_architectures(${TEST_TARGET_NAME} ARCHITECTURES ${GINKGO_CUDA_ARCHITECTURES}
                                UNSUPPORTED "20" "21")
  set_target_properties(${TEST_TARGET_NAME} PROPERTIES OUTPUT_NAME ${test_name})
...
function(ginkgo_create_mpi_test test_name num_mpi_procs)
  ...
  add_test(NAME ${REL_BINARY_DIR}/${test_name} COMMAND ${MPIEXEC_EXECUTABLE} ${test_param})
...
function(ginkgo_create_hip_test test_name)
  ...
  target_include_directories(
    "${TEST_TARGET_NAME}"
    PRIVATE "$<BUILD_INTERFACE:${Ginkgo_BINARY_DIR}>"
            # Only `math` requires it so far, but it's much easier to put these this way.
            ${GINKGO_HIP_THRUST_PATH}
            # Only `exception_helpers` requires this so far, but it's much easier to put these this
            # way.
  ...
  # GINKGO_RPATH_FOR_HIP needs to be populated before calling this for the linker to include our
  # libraries path into the executable's runpath.
