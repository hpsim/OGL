...
TEST(Combination, CanCreateCombinationMatrixWithSparseRealMatrix)
{
    ...
    int dim_size = 5;
    auto dim = gko::dim<2>{static_cast<gko::size_type>(dim_size),
                           static_cast<gko::size_type>(dim_size)};

    auto exec = gko::share(gko::ReferenceExecutor::create());

    auto m1linop =
        gko::share(gko::read<mtx>(std::ifstream("data/A_sparse.mtx"), exec));
    ...
    // Assert
    ASSERT_EQ(cmb->get_size(), dim);
    ASSERT_EQ(cmb->get_coefficients().size(), 2);
    ASSERT_EQ(cmb->get_operators().size(), 2);

    // Act
    auto x = gko::matrix::Dense<scalar>::create(
        exec, gko::dim<2>{static_cast<gko::size_type>(dim_size), 1});
    x->fill(0.0);
    cmb->apply(b, x);
...
TEST(Combination, CanCreateCombinationMatrixWithOpenFOAMLDUMatrix)
{
    ...
    auto Llinop = gko::share(gko::read<mtx>(std::ifstream("data/L.mtx"), exec));

    auto Dlinop = gko::share(gko::read<mtx>(std::ifstream("data/D.mtx"), exec));

    auto Ulinop = gko::share(gko::read<mtx>(std::ifstream("data/U.mtx"), exec));
...
TEST(Combination, CanConvertToCoo)
{
    ...
    auto dim = gko::dim<2>{static_cast<gko::size_type>(dim_size),
                           static_cast<gko::size_type>(dim_size)};
    gko::matrix_data<double, int> m1(dim, {{0, 0, 2}, {1, 1, -3}, {2, 3, 5}});
    gko::matrix_data<double, int> m2(dim, {{3, 2, 4}, {3, 3, 1}, {4, 4, 2}});

    auto exec = gko::share(gko::ReferenceExecutor::create());
    auto m1linop = gko::share(InputFormat::create(exec, dim));
    m1linop->read(m1);
    ...
    ASSERT_EQ(out->get_values()[1], -3);
    ASSERT_EQ(out->get_values()[2], 5);
    ASSERT_EQ(out->get_values()[3], 4);
    ASSERT_EQ(out->get_values()[4], 1);
    ASSERT_EQ(out->get_values()[5], 2);

    // Verify row indices
    ASSERT_EQ(out->get_row_idxs()[0], 0);
    ASSERT_EQ(out->get_row_idxs()[1], 1);
    ASSERT_EQ(out->get_row_idxs()[2], 2);
    ASSERT_EQ(out->get_row_idxs()[3], 3);
    ASSERT_EQ(out->get_row_idxs()[4], 3);
    ASSERT_EQ(out->get_row_idxs()[5], 4);

    // Verify column indices
    ASSERT_EQ(out->get_col_idxs()[0], 0);
    ASSERT_EQ(out->get_col_idxs()[1], 1);
    ASSERT_EQ(out->get_col_idxs()[2], 3);
    ASSERT_EQ(out->get_col_idxs()[3], 2);
    ASSERT_EQ(out->get_col_idxs()[4], 3);
    ASSERT_EQ(out->get_col_idxs()[5], 4);
}
...
TEST(Combination, CanConvertToCsr)
{
    ...
    using ValueType = scalar;
    using IndexType = label;
    using InputFormat = gko::matrix::Coo<ValueType, IndexType>;
    using OutputFormat = gko::matrix::Csr<ValueType, IndexType>;
    ...
    // Verify row indices
    ASSERT_EQ(out->get_row_ptrs()[0], 0);
    ASSERT_EQ(out->get_row_ptrs()[1], 1);
    ASSERT_EQ(out->get_row_ptrs()[2], 2);
    ASSERT_EQ(out->get_row_ptrs()[3], 3);
    ASSERT_EQ(out->get_row_ptrs()[4], 5);
}

TEST(Combination, CanMoveToCoo)
{
    // Arrange
    using ValueType = scalar;
    using IndexType = label;
