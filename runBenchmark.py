...
class Case:
    ...
    def create(self):
        ...
        ensure_path(self.path)
        self.copy_base(self.path, self.base_case_path)
        self.set_matrix_solver("boxTurb16")
...
def build_parameter_study(test_path, results, executor, solver, setter):
    for (e,s,n) in product(executor, solver, setter):
        ...
        path = test_path / Path(e[0] + e[1]) / str(n.value)
        ...
        if exist and not clean:
            skip = True
        is_base_case=False
        base_case_path=test_path /  Path(e[0] + e[1]) / Path("p")
        if e[0] == "base":
            is_base_case=True
        if not skip:
            case = Case(
...
if __name__ == "__main__":
    ...
    print("Meshing")
    check_output(["blockMesh"], cwd=test_path)
...
def build_parameter_study(test_path, results, executor, solver, setter):
    for (e,s,n) in product(executor, solver, setter):
        ...
        if exist and not clean:
            skip = True
        is_base_case=False
        base_case_path=test_path / "base" / str(n.value) / "boxTurb16"
