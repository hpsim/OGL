                                                   ...
                                                   word("reference")))
{
    ...
    if (app_exec_stored) {
        ref_exec_ptr_ =
            &get_db().lookupObjectRef<GKOReferenceExecPtr>(app_executor_name_);
    ...
    if (device_exec_stored) {
        ...
        if (device_executor_name_ == "hip") {
            hip_exec_ptr_ =
                &get_db().lookupObjectRef<GKOHipExecPtr>(device_executor_name_);
            return;
        }
    }
    ...
    ref_exec_ptr_ =
        new GKOReferenceExecPtr(IOobject(app_exec_store, get_db()),
    ...
    if (device_executor_name_ == "omp") {
        omp_exec_ptr_ = new GKOOmpExecPtr(IOobject(device_exec_store, get_db()),
